...
package body Controller_Quit is
   ...
   begin
      if ToolboxWindow.Is_Open(main_objectid) then
         if not FileExternal.Exists ("<Choices$Write>.InsertIcon") then
            FileExternal.Create_Directory ("<Choices$Write>.InsertIcon");
         end if;

         ToolboxWindow.Get_WindowPosition (main_objectid,x_pos,y_pos);
         declare
            Target : FileHandle_Type(new UString'(U("<Choices$Write>.InsertIcon.Misc")),Write);
         begin
